...
class Mclient
{
        ...
        try {
            ...
            $this->client->setCallback(function (Request $request) {
                $this->responses[] = [
                    ...
                    "body" => $request->getResponseText() ?? '',
                    "headers" => $this->lines_to_headers($request->getResponseHeaders() ?? ''),
    ...
    private function generateRequests(): void
    {
        foreach ($this->requests as $request) {
            ...
            $curl[CURLOPT_SSL_VERIFYPEER] = $request["options"]["verify"] ?? false;
            $curl[CURLOPT_SSL_VERIFYHOST] = $request["options"]["verify"] ?? false;
            ...
            if (empty($request["headers"]["user-agent"]))
                $request["headers"]["user-agent"] = "Mclient";
